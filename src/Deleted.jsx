...
function Deleted() {
    ...
    return (
        <><section className="main ">
            <div className="heading">Deleted Items</div>
            <div className="notes-container">
                {data.map((elem, index) => {
                    return (
                        <>
                            <div className="note-wrapper" id={`card${index}`} key={`card${index}`}>
                                <div className="note-card" key={index}>
                                    ...
                                    <div className="note-settings" style= {{display:showMenu?"block":"none"}}>
                                        <ul>
                                            ...
                                            <li onClick={()=>{
                                                firebase.firestore().collection(usercookie[1]).doc(elem.id).delete();
                                                notify("Please refresh to see update")
                                                setShowMenu(false)

                                            }}><i className="fas fa-check-square"></i> Delete</li>

                                            <li onClick={()=>{
                                                const docRef = firebase.firestore().collection(usercookie[1]).doc(elem.id);
                                                docRef.update({status: "undone"});
                                                notify("Please refresh to see update")
                                                setShowMenu(false)

                                            }}><i className="fas fa-trash-restore"></i> Restore</li>
